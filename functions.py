...
def get_chemical_details(query: str):
    ...
    if chemical_name == "none":
        ...
        params = {'serviceKey': os.getenv("KOSHA_API_KEY"), 'searchWrd': chemical_name, 'searchCnd': '0',
                  'numOfRows': '10'}
...
tool_dict = {"get_accident_records": get_accident_records, "get_chemical_usage": get_chemical_usage,
             "get_risk_assessment": get_risk_assessment, "get_regulations_data": get_regulations_data,
             "get_chemical_details": get_chemical_details, "dynamic_risk_assessment": dynamic_risk_assessment}
        ...
        with st.chat_message("ai"):
            with st.spinner("Thinking..."):
                ...
                if st.session_state.conversation_history:
                    ...
                    recent_history = st.session_state.conversation_history[-2:] if len(
                        st.session_state.conversation_history) >= 3 else st.session_state.conversation_history
                    history_context = "\n".join(
                        [f"Previous: User: {h['User']} | AI: {h['AI']}" for h in recent_history])
                    ...
                    for tool_call in ai_msg.tool_calls:
                        ...
                        if tool_name == "get_chemical_details":
                            ...
                            if "table" in user_query.lower() or "테이블" in user_query.lower():
                                ...
                                json_data = risk_assessment_table_output(risk_assessment_docs=risk_assessment_docs,
                                                                         query=context_query)
                                if json_data:
                                ...
                                output = risk_assessment_output(risk_assessment_docs=risk_assessment_docs,
                                                                query=context_query)
                        elif tool_name == "dynamic_risk_assessment":
                            risk_assessment_docs = dynamic_risk_assessment(context_query)
                            output = dynamic_risk_assessment_output(risk_assessment_docs=risk_assessment_docs,
                                                                    query=context_query)
                        else:
                            # Handle unknown tool
